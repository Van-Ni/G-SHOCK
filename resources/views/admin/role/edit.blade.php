...
@section('content')
    <div id="content" class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.role.update',['id' => request()->id]) }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="name">Tên vai trò</label>
                        <input class="form-control" value="{{ $infoRole->role_name }}" type="text" name="name" id="name">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" value="{{ $infoRole->display_name }}" type="text" name="display_name" id="display_name">
                        @error('display_name')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                                ...
                                <tr>
                                    @foreach ($parent->permissionChildrens as $child)
                                        <td><input value="{{ $child->id }}" class="checkboxAll" type="checkbox"
                                                ...
                                                {{ $permission->contains('id',$child->id) ? "checked" : "" }}
                                                >
                                            <label for="{{ $child->id }}"
